...
public class Computeclass {
    ...
    public static double SimilarDegree(String strA, String strB){     
        ...
        // 如果结果和strA长度完全一致，认为是100%;
        if(temp2 <= newStrA.length()){
        	 return temp2 * 1.0 / newStrA.length();      
  ...
  * 计算相似度 
  * */  
 public static double SimilarDegree2(String strA, String strB){     
     String newStrA = removeSign(strA);      
     String newStrB = removeSign(strB);  
     //用较大的字符串长度作为分母，相似子串作为分子计算出字串相似度  
...
//     }
     ...
     * 因为对符号进行相似度比较没有实际意义，故符号不加入考虑范围。 
     * */  
    public static boolean charReg2(char charValue) {      
    ...
    public static String longestCommonSubstring(String strA, String strB) {     
        ...
        if(chars_strA.length < chars_strB.length){
        	char[] temp = chars_strA;
        	chars_strA = chars_strB;
        	chars_strB = temp;
        }
